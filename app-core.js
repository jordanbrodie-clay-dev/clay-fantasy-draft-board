(() => {
  ...
  function renderDraftRoom() {
    ...
    <div class="positionplans"><div class="eyebrow">Relative positional value · draft tiers, not rounds</div><div class="positiongrid">${result.positionPlans.map((item) => { const tierNames = item.currentTierPlayers.slice(0, 3).map((player) => esc(player.n)).join(" · "); const actionClass = item.tierAction === "DRAFT THIS TIER" ? "tier-now" : item.tierAction === "STACK" ? "tier-stack" : item.tierAction === "FILLED" || item.tierAction === "DEPTH ONLY" ? "tier-muted" : "tier-wait"; return `<div class="positioncard"><div class="tierhead"><span class="draftbadge">${item.position}</span><span class="tieraction ${actionClass}">${item.tierAction}</span></div><div class="tierflow"><b>T${item.currentTier} now</b><span>→</span><b>${item.projectedTier == null ? "—" : `T${item.projectedTier} next`}</b><span>→</span><b>${item.projectedTwoTier == null ? "—" : `T${item.projectedTwoTier} in 2`}</b></div><p class="tierplayers">${tierNames}</p><p><b>${pct(item.currentTierSurvival)}</b> of T${item.currentTier} reaches pick ${item.nextPick} · <b>${pct(item.currentTierTwoPickSurvival)}</b> survives two turns</p><p class="tiernext"><b>${item.tierValueDrop.toFixed(2)}σ</b> relative position value lost by waiting · best now ${esc(item.player.n)}</p></div>`; }).join("")}</div></div>`;
  }
