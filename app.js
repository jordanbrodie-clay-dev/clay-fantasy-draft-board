(() => {
  ...
  function view() {
    ...
    const key = f.sort;
    const ascending = ["r", "adp", "ecr", "qr"].includes(key);
    players.sort((a, b) => {
  ...
  function renderDraftRoom() {
    ...
    const explanation = alternative
      ? `${decisionPrefix}${pct(top.survival)} chance he then reaches pick ${top.nextPick}. If you wait at ${top.player.p === "DEF" ? "D/ST" : top.player.p}, the best likely alternative is ${alternative.n}; the modeled position drop is ${drop.toFixed(2)}σ.`
      : `${decisionPrefix}The model sees no dependable ${top.player.p} alternative at pick ${top.nextPick}; this is a positional-cliff selection.`;
    body.innerHTML = `<div class="recmain">
      ...
      <div class="recmetrics"><div class="recmetric"><b>${top.value.toFixed(2)}σ</b><span>player quality</span></div><div class="recmetric"><b>${pct(top.survival)}</b><span>there next pick</span></div><div class="recmetric"><b>${drop.toFixed(2)}σ</b><span>position drop if waiting</span></div><div class="recmetric"><b>${n1(top.player.adp)}</b><span>Sleeper ADP</span></div></div>
